...
DESCRIPTION
       ...
       signal()  function,  and  whose values shall compare unequal to the ad‐
       dress of any declarable function.
       ...
       ILL, SIGINT, SIGSEGV, and SIGTERM to be defined. An implementation need
       not  generate  any of these six signals, except as a result of explicit
       use of interfaces that generate signals, such as raise(),  kill(),  the
       General  Terminal  Interface  (see Section 11.1.9, Special Characters),
       and the kill utility, unless otherwise stated (see,  for  example,  the
       System  Interfaces volume of POSIX.1‐2017, Section 2.8.3.3, Memory Pro‐
       tection).

       The following signals shall be supported on  all  implementations  (de‐
       fault actions are explained below the table):
  ...
  └──────────┴────────────────┴────────────────────────────────────────────────────┘
       ...
       The effects on the process in each case are described in the System In‐
       terfaces volume of POSIX.1‐2017, Section 2.4.3, Signal Actions.
       ...
       The <signal.h> header shall define the following macros which shall ex‐
       pand to integer constant expressions that need not  be  usable  in  #if
...
└───────┴─────────────┴──────────────────────────────────────────────────────────────────┘
       ...
       plementations do not generate a different value from the ones described
       in this list for circumstances described in this list.
