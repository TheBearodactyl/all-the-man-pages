...
DESCRIPTION
       ...
       ino_t, mode_t, nlink_t, uid_t, gid_t, off_t, and time_t  types  as  de‐
       scribed in <sys/types.h>.

       The  <sys/stat.h>  header  shall  define  the timespec structure as de‐
       scribed in <time.h>.  Times shall be given in seconds since the Epoch.
       ...
       lstat(), and stat() in the System Interfaces volume of POSIX.1‐2017.
       ...
       The  <sys/stat.h>  header shall define the following symbolic constants
       for the file mode bits encoded in type mode_t, with the  indicated  nu‐
       meric  values.  These  macros shall expand to an expression which has a
       ...
       ture. The macro shall evaluate to a non-zero value if the specified ob‐
       ject is implemented as a distinct file type and the specified file type
       is  contained  in the stat structure referenced by buf.  Otherwise, the
       ...
       Inclusion  of the <sys/stat.h> header may make visible all symbols from
       the <time.h> header.
...
RATIONALE
       ...
       bolic values for that type can be used. An alternative would be to  re‐
       quire that these constants are also defined by including <sys/types.h>.
       ...
       not update the time entry fields for functions listed in the System In‐
       terfaces  volume of POSIX.1‐2017 unless the standard requires that they
       do, except in the case of documented extensions to the standard.
...
SEE ALSO
       ...
       The  System  Interfaces  volume of POSIX.1‐2017, chmod(), fchmod(), fs‐
       tat(), fstatat(), futimens(), mkdir(), mkfifo(), mknod(), umask()
