...
REGEX SYNTAX
       ...
       syntax: https://docs.rs/regex/*/regex/#syntax

       ripgrep uses byte-oriented regexes, which has some additional
       documentation: https://docs.rs/regex/*/regex/bytes/index.html#syntax
       ...
       "extended" (ERE) regular expressions supported by egrep, but with a few
...
POSITIONAL ARGUMENTS
       ...
       PATH
           ...
           recursively. File paths specified explicitly on the command line
           override glob and ignore rules.

OPTIONS
       Note that many options can be disabled via flags. In some cases, those
       flags are not listed in a first class way below. For example, the
       --column flag (listed below) enables column numbers in ripgrep’s
       output, but the --no-column flag (not listed below) disables them. The
       reverse can also exist. For example, the --no-ignore flag (listed
       below) disables ripgrep’s gitignore logic, but the --ignore flag (not
       listed below) enables it. These flags are useful for overriding a
       ripgrep configuration file on the command line. Each flag’s
       ...
       --context-separator SEPARATOR
           ...
           (-A, -B or -C). Escape sequences like \x7F or \t may be used. The
           default value is --.
       ...
       -c, --count
           ...
           total number of matches, unless -U/--multiline is enabled. In
           multiline mode, --count is equivalent to --count-matches.
           ...
           to force printing the file path in this case. If you need a count
           to be printed regardless of whether there is a match, then use
           --include-zero.
       ...
       --field-context-separator SEPARATOR
           ...
           including zero. Escape sequences like \x7F or \t may be used. The
           default value is -.

       --field-match-separator SEPARATOR
           Set the field match separator, which is used to delimit file paths,
           line numbers, columns and the match itself. The separator may be
       ...
       -l, --files-with-matches
           Print the paths with at least one match and suppress match
           contents.
       ...
       -g, --glob GLOB ...
           ...
           is equivalet to -g abc -g abd. Empty alternatives like -g ab{,c}
           are not currently supported. Note that this syntax extension is
           also currently enabled in gitignore files, even though this syntax
           isn’t supported by git itself. ripgrep may disable this syntax
           extension in gitignore files, but it will always remain available
           via the -g/--glob flag.
       ...
       --no-ignore
           ...
           implies --no-ignore-dot, --no-ignore-exclude, --no-ignore-global,
...
EXIT STATUS
       ...
       the --quiet flag and a match was found. In summary:
...
AUTOMATIC FILTERING
       ...
       One of ripgrep’s most important features is its automatic smart
       filtering. It is the most apparent differentiating feature between
       ripgrep and other tools like grep. As such, its behavior may be
       surprising to users that aren’t expecting it.
...
CAVEATS
       ...
       -a flag enabled. (When the -a flag isn’t enabled, ripgrep will replace
       all NUL bytes with line terminators, which typically prevents
       exorbitant memory usage.) Thirdly, when ripgrep searches a large file
       using a memory map, the process will report its resident memory usage
       as the size of the file. However, this does not mean ripgrep actually
       needed to use that much memory; the operating system will generally
       handle this for you.
